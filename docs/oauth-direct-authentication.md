...
You may authenticate user via this work flow using [[\algsupport\authclient\OAuth2::authenticateUser()]].
For example:
...
You may authenticate client only via this work flow using [[\algsupport\authclient\OAuth2::authenticateClient()]].
For example:
...
```php
use algsupport\authclient\clients\Google;
use algsupport\authclient\signature\RsaSha;
